...
Adversarial code review (Blind Hunter + Edge Case Hunter + Acceptance Auditor, all Opus) of commit `7d4d84c` vs baseline `8f4cbb1` — 2026-06-29. Acceptance Auditor verdict: **ACCEPT-WITH-NITS** (all 3 ACs + ADs honored; clean 4.3/4.4 boundary; no FX leakage; build/vet/test/nofloat green). No High. 5 patch, 2 defer, 4 dismissed.

- [x] [Review][Patch][Med] Price entered with >4 dp is silently re-rounded by Postgres `NUMERIC(19,4)` (half-away-from-zero, not banker's), and a positive price `< 0.00005` rounds to `0` → raw `CHECK (price > 0)` violation echoed to the user [internal/service/price/price.go Add] — round the parsed price to `money.MoneyScale` (banker's) then re-check `IsPositive()` so ingestion uses the same rounding as valuation and sub-cent inputs get the typed `ErrNonPositivePrice` instead of a raw DB error (blind+edge).
- [x] [Review][Patch][Med] Unrealized G/L sign/colour is derived from the UNROUNDED value while the displayed figure is rounded → a break-even or fractional-share holding (cost basis carries sub-cent crumbs from `DeriveHoldings`) is painted loss-red and `market − basis` can disagree with the shown unrealized by a cent; the `ValueHolding` comment "CostBasis is already at money scale" is false [internal/domain/valuation.go ValueHolding, internal/http/router.go renderInvestmentDetail] — round the unrealized gain to money scale, derive the sign from the rounded value, fix the comment (edge+blind).
- [x] [Review][Patch][Low] Break-even (exactly `0`) unrealized G/L renders in gain-green, not neutral [internal/http/router.go + web/pages.templ] — only colour non-zero values (blind+edge).
- [x] [Review][Patch][Low] `/prices` list renders a bare `decimal.String()` (`16`, trailing zeros stripped, no currency), inconsistent with the `NN.NNNN CUR` money format used on the holdings page; the page never shows which currency each price is in [internal/http/router.go renderPrices, web/shell.go PriceRow, db/query/price.sql ListPrices] — carry the security `quote_currency` through `ListPrices`/`price.Price` and render money-formatted (blind+edge+auditor).
- [x] [Review][Patch][Low] Weak test assertions — `strings.Contains(body, "16")` and `strings.Contains(b, "—")` can pass on unrelated content [internal/http/router_test.go TestPricesAddAndList, TestHoldingValuationColumns] — assert the full rendered token / the specific cell (blind).
- [x] [Review][Defer] `renderPrices` swallows `Prices.List`/`Securities.List` errors → empty page (200) on a DB outage [internal/http/router.go renderPrices] — deferred, identical to the project-wide swallow pattern already tracked from 4.1/4.2 (accounts/categories/exchange-rates/securities); fix across the `http` layer at once.
- [x] [Review][Defer] Raw wrapped DB error echoed to the client via `err.Error()` on a non-typed insert failure [internal/http/router.go pricesSubmit] — deferred, same project-wide pattern from 4.1/4.2 (the [Med] price-rounding patch above removes the common sub-cent bad-input raw-error path).

Dismissed as noise/non-issues: `service/price.PriceAt`/`LatestPrices` "dead in prod" (intentional forward-scaffolding for 4.4/5.3, exactly like 4.1's `GetSecurityBySymbol`; `Holdings` reads the store directly by the store-not-service rule); future-dated prices have no UI "not yet effective" cue (behavior is correct per AC2 — most-recent ≤ today wins, future rows simply wait); no runtime currency guard inside `ValueHolding` (safe by construction — `security.quote_currency` is immutable and the 4.2 trade form is currency-filtered, so a holding's security currency always equals its account currency; the Auditor confirmed); Blind Hunter's "no import hunk for `domain`/`fmt`/`time` in `transaction.go`" (false positive — those imports already existed, a blind-review artifact).
...
Modified: `internal/service/transaction/transaction.go` (`HoldingView` + price fields; `Holdings` loads `store.LatestPrices` and calls `domain.ValueHolding`) + `trade_test.go`; `internal/http/router.go` (`Prices` iface + `Deps` field, `/prices` GET/POST + `pricesForm`/`pricesSubmit`/`renderPrices`, `renderInvestmentDetail` HoldingRow price mapping) + `router_test.go` (stub `Prices`, extended `Holdings` stub); `cmd/server/main.go` (wire `price.New(pool)`); `web/pages.templ` (`PricesPage` + Settings link + holdings-table price columns) + rebuilt `app.css`; `web/shell.go` (`PriceRow`, `HoldingRow` price fields); `README.md`. No structural variance — `service/price` is a new sibling package following the established onion layout; `/prices` is reference-data management linked from `/settings` like `/exchange-rates`.
...
| 2026-06-29 | Code review (3 adversarial layers, Auditor ACCEPT-WITH-NITS): applied 5 patches — price rounded to money scale (banker's) at ingest so a sub-cent input gives `ErrNonPositivePrice` not a raw CHECK error (Med); unrealized G/L rounded to money scale + sign derived from the rounded value so a fractional-crumb/break-even holding isn't painted red (Med); zero unrealized now neutral (not green); `/prices` list money-formatted with the security's quote currency (`ListPrices` + `price.Currency`); two weak test assertions tightened; `LatestPrices` compares `$1::date` for tz-stable "today". Added `TestValueHoldingSubCentBasisIsNeutral` + price-rounding asserts. 2 findings deferred (project-wide render-swallow / raw-err echo), 4 dismissed. build/vet/test/nofloat green. Status → done. |
